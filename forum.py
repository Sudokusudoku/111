...
def register():
    ...
    # check whether two passwords are the same
            if password1!=password2:
                ...
                info = Information(user_id = user.id, number_of_followed=0,number_of_following=0)
                info.owner = user
                db.session.add(user)
                db.session.add(info)
                db.session.commit()
...
def question():
    if request.method == 'GET':
        ...
        if 'attach_file'in request.files: #如果用户上传了文件
            filename = docs.save(request.files['attach_file'])
            question = Question(title=title, file = filename ,content=content,report_reasons_and_times = 'A@0|B@0|C@0|D@0', report_total = 0, type = kind)
        else:
...
def detail(question_id):
    question_model = Question.query.filter(Question.id==question_id).first() #这条帖子
    ...
    if session.get('user_id'):
        ...
        follow = Following.query.filter(Following.user_id == user_id, Following.followed_user_id == author_id).first()
        file = question_model.file
        if follow and file: #传了文件、关注了
            return render_template('detail.html', file = 1,yes = 1, user=user, info = info, author_user = author_user, question=question_model)
        elif file and not follow: #传了文件、没关注
            return render_template('detail.html', file = 1, user=user, info=info, author_user=author_user, question=question_model)
        elif follow and not file: #没传文件、关注了
            return render_template('detail.html', yes = 1, user=user, info=info, author_user=author_user, question=question_model)
        else:    #没传文件、没关注
...
def edit():
    if request.method == 'GET':
        user_id = session.get('user_id')
        user = User.query.filter(User.id == user_id).first()
        return render_template('edit_personal_detail.html', user = user)
    else:
        user_id = session.get('user_id')
        user = User.query.filter(User.id == user_id).first()
...
def follow(person_id):
    user_id = session.get('user_id') #登录者
...
#举报某用户
@app.route('/report/<id>/<num>/<answer>', methods=['POST','GET'])
def report(id, num, answer):
    if request.method == 'GET':
        return render_template('register.html')
    else:
        user_id = session.get('user_id')
        ...
        author = question_model.author_id
        info = Information.query.filter(Information.user_id == author).first()
        ...
        # 若被举报的是回答，则num=1
            ...
            if report_unique:
                ...
                answer_model = Answer.query.filter(Answer.id == answer).first()
                ...
                if total_num>50:
                    ...
                    user.report_time = datetime.now()
                    db.session.commit()
        return render_template('detail.html', question=question_model, info = info, user = user)

#显示其他人的个人主页
@app.route('/elseinfo/<user_id>/')
@login_required
def elseinfo(user_id):
    else_user = User.query.filter(User.id == user_id).first() #要看的人
    else_user_id = else_user.id
    info_model = Information.query.filter(Information.user_id == user_id).first()
    ...
    login_user_id = session.get('user_id') #登录的人的id
    user = User.query.filter(User.id == login_user_id).first()
    follow = Following.query.filter(Following.user_id == login_user_id, Following.followed_user_id == else_user_id).first()
    if follow: #关注了
        return render_template('others_information.html', yes = 1, **questions, user=user, else_user = else_user, info=info_model,time=session.get('login_time'))
    else:
